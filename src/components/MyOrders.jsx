...
export default function MyOrders() {
    ...
    if (loading) return <div style={{ padding: 48, color: "var(--text-muted)" }}>{t("orders.loading")}</div>;
                        ...
                        {order.order_status !== "cancelled" && (
                            <div style={{ marginTop: 10, fontSize: 13 }}>
                                {order.order_status === "ready_for_pickup" && order.final_delivery_date && (
                                    <span style={{ background: "var(--alert-success-bg)", color: "var(--alert-success-text)", padding: "3px 10px", borderRadius: 12, fontWeight: 600 }}>
                                ...
                                {["pending", "confirmed"].includes(order.order_status) && !order.all_items_available && (
                                    <span style={{ background: "var(--alert-warning-bg)", color: "var(--alert-warning-text)", padding: "3px 10px", borderRadius: 12 }}>
